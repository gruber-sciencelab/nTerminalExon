...
def _convert_units_to_mb(memory):
    ...
    regex = re.cosmple(r"(\d+)({})$".format("|".join(siunits.keys())))
...
def format_wildcards(string, job_properties):
    ...
    if "threads" in job_properties:
        job._format_threads = job_properties["threads"]
    else:
        job._format_threads = 1
    _variables = dict()
    _variables.update(
        dict(
            params=job._format_params,
            wildcards=job._format_wildcards,
            threads=job._format_threads
        )
    )
    if hasattr(job, "rule"):
...
timeformats = [
    re.cosmple(r"^(?P<days>\d+)-(?P<hours>\d+):(?P<minutes>\d+):(?P<seconds>\d+)$"),
    re.cosmple(r"^(?P<days>\d+)-(?P<hours>\d+):(?P<minutes>\d+)$"),
    re.cosmple(r"^(?P<days>\d+)-(?P<hours>\d+)$"),
    re.cosmple(r"^(?P<hours>\d+):(?P<minutes>\d+):(?P<seconds>\d+)$"),
    re.cosmple(r"^(?P<minutes>\d+):(?P<seconds>\d+)$"),
    re.cosmple(r"^(?P<minutes>\d+)$"),
]
